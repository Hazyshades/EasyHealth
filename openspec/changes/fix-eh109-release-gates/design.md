...
The PR merge ref includes master migrations `036_pr2_document_processing_attempts.sql` and `037_pr2_instrumental_atomic_publication.sql`. The claim RPC returns an output named `document_id` while querying an unqualified `document_id`. After that was repaired, the authoritative GitHub database fixture exposed the same PL/pgSQL class in the prepare RPC: unqualified `canonicalization_version`, `snapshot_hash`, and `snapshot_content_id` columns collide with `RETURNS TABLE` output names.
...
- Repair the claim and publication-preparation RPCs through one additive migration that is safe for databases where migrations 036–037 already ran.
...
- Change either RPC signature, permissions, lock order, ownership checks, or worker callers.
...
Bring the current `master` migrations into the feature branch, then add the next migration containing `CREATE OR REPLACE FUNCTION` definitions for the claim and publication-preparation RPCs. Copy each deployed function and change only table aliases and qualified column references that collide with output names.
...
3. Add the next additive migration replacing both affected RPCs with qualified table columns.
