...
Order the selected numeric points by `observedAt` ascending, then immutable `observationId` ascending using the canonical UUID byte order. Use that same order for the point ledger and first/latest selection, so equal timestamps cannot change statistics or direction across reads or exports. If fewer than two points remain, direction is `not_available` with a comparison-unavailable limitation before policy lookup or delta classification. For two or more points, direction compares the first and latest points. Let `delta = latest - first` and `threshold = max(absolute, relative * abs(first))`. If `abs(delta) <= threshold`, direction is `stable` (the boundary is inclusive); if `delta > threshold`, it is `increasing`; if `delta < -threshold`, it is `decreasing`. If no reviewed policy entry exists, direction is `not_available` with a limitation; there is no arbitrary zero/default tolerance. The DTO never emits `improving` or `worsening`; those labels require a future domain rule with Registry ownership.
...
The `/api/biomarkers/dynamics` route resolves the authenticated profile and `profile_current` scope before returning the DTO. The EH-148 report handoff invokes the same adapter with immutable `report_scope_document_ids`. The projection accepts only canonical `YYYY-MM-DD` `start`/`end` values with `start <= end`, applies an inclusive UTC-calendar-date range to observed dates (including timestamps anywhere on the end date), and represents undated rows outside the dynamics series with a limitation. A client may choose a period preset through `biomarker_dynamics_period` but cannot inject arbitrary observations, widen report scope, or perform its own conversion.
