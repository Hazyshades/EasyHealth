...
- [ ] 1.1 Implement owner and EH-151 share adapters that load only validated EH-148 content, its complete report-scope source ledger, and an optional frozen EH-149 `BiomarkerDynamicsReport` DTO; keep raw-document child scope separate.
...
- [ ] 2.2 Implement CSV `record_type`/`row_order` rows for metadata, claims, complete source ledger entries, and measurements; measurement rows come from the supplied dynamics DTO with source IDs, document IDs, native/display values and units, ranges, dates, and conversion indicators.
