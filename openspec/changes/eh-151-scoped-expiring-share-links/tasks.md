...
- [ ] 1.1 Add the share-link, explicit document-scope, minimized access-event, rate-limit-bucket, and `share_replacement_operations` migrations with profile ownership, expiry, revocation, `download_policy`, `allowed_export_formats`, a 1–128 printable-ASCII owner/share-scoped idempotency key, unique predecessor and `(profile_id, predecessor_share_id, idempotency_key)` constraints, expiry indexing, the `(key_digest, window_started_at)` rate-limit conflict key, and service-only RPCs for replacement, access-event cleanup, atomic rate-limit consumption, and bounded rate-limit-bucket cleanup.
...
- [ ] 2.2 Implement optional salted slow PIN hashing plus `src/lib/share-links/trusted-ingress.ts` and `rate-limit.ts` over the shared Supabase Postgres adapter: private/mTLS ingress, stripped/signed `X-EH-Edge-*` assertion verified with `SHARE_TRUSTED_PROXY_ATTESTATION_KEY`, `SHARE_TRUSTED_PROXY_CIDRS`, and `SHARE_TRUSTED_PROXY_ATTESTATION_MAX_AGE_SECONDS`, strict single-address parsing and spoofed-header rejection, HMAC-derived token/requester keys, atomic fixed-window limits `10/60s` and `30/60s`, explicit environment settings, generic `429` exhaustion, generic `503` store/proxy/configuration failure, and no local fallback.
...
- [ ] 3.1 Add focused route fixtures for scope, profile isolation, token failures, PIN retries, expiry, revoke, repeated token/PIN failures, unavailable rate-limit store, missing/malformed/expired signed trusted-ingress assertion, spoofed forwarding headers, deleted-report cascade, archived/deleted cited-source read resolution, cache headers, raw-download denial, and revoke/expiry after a prior raw-download request.
