...
### Requirement: Non-URL PIN proof lifecycle

For a share configured with a PIN, the public boundary SHALL expose exactly `POST /api/share/[token]/pin` for proof establishment. The request SHALL accept JSON `{ "pin": string }` in the body only; PIN material SHALL be rejected from URL paths, query parameters, headers, cookies, and response fields. After valid token/share verification and a correct PIN, the route SHALL persist only a keyed digest of a cryptographically random proof with the exact share ID and bounded expiry, set the `__Host-eh-share-pin` cookie with `Secure`, `HttpOnly`, `SameSite=Strict`, `Path=/`, and no `Domain`, return no token/PIN/proof in its body, and apply `Cache-Control: no-store`. `requireSharePinProof` SHALL verify that cookie after token verification and before reading report data, export bytes, or raw-document bytes for the page, API, EH-153 export adapter, and raw-document proxy. It SHALL bind the proof to the exact active share, recheck share expiry/revocation and proof expiry, clear invalid proofs, and return one generic PIN-required/unavailable outcome for missing, wrong, expired, revoked, cross-share, or malformed proofs. The proof TTL and `SHARE_PIN_PROOF_PEPPER` SHALL use the deployed EH-151 settings; no proof, cookie, PIN, or request body value may enter logs, telemetry, access events, URLs, referrers, or public response fields. Expired proof rows SHALL be removed by the bounded worker cleanup.

#### Scenario: Correct PIN establishes a reusable scoped proof

- **WHEN** a recipient submits the correct PIN to `POST /api/share/[token]/pin` as a JSON body
- **THEN** the server stores only the keyed proof digest bound to that share and sets the protected `__Host-eh-share-pin` cookie
- **AND** the response body contains no token, PIN, or proof and is not cacheable
- **AND** later page, API, export, and raw-document requests use the same proof verifier before bytes are read

#### Scenario: PIN proof failures do not expose content

- **WHEN** the proof cookie is missing, malformed, wrong for the share, expired, or the share is revoked/expired
- **THEN** every public resource route returns the same generic PIN-required/unavailable outcome
- **AND** it clears the invalid proof cookie and emits no report, export, or raw-document bytes

#### Scenario: PIN material stays out of public and diagnostic channels
...
#### Scenario: Durable tombstone blocks a public share
- **WHEN** the durable deletion transition commits a source document tombstone for a report's actual source set
- **THEN** the share route returns generic unavailable before reading report snapshots or issuing bytes
- **AND** final purge removes the report/share graph through the owner report-delete transition
...
The service-only `public.replace_report_share` RPC SHALL accept the authenticated owner/profile, predecessor share ID, a bounded opaque owner/share-scoped `idempotency_key` constrained to 1–128 printable ASCII bytes from `[A-Za-z0-9._~-]`, a server-minted keyed token digest, and token-key version. It SHALL lock the predecessor, first look up an existing committed operation for the owner/predecessor/key and return only recorded operation/successor metadata on replay, then for a new key verify ownership and active state, reserve `share_replacement_operations` with unique `(profile_id, predecessor_share_id, idempotency_key)`, enforce one committed successor per predecessor, copy immutable report/raw-document scope, expiry, and export policy, revoke the predecessor, and commit the operation atomically. Operation rows SHALL contain no plaintext token/PIN. A replay with the same key SHALL return only recorded operation/successor metadata and no plaintext link; a different key after predecessor revocation SHALL return a generic conflict; and any mint, reservation, ownership, or RPC failure SHALL roll back the operation and leave the predecessor active.
...
After a token selects a share and the route makes a public authorization decision, EH-151 SHALL write a minimized event to the durable `report_share_access_events` store with non-null share ID, event time, result, resource kind, coarse client class, and `retention_expires_at = occurred_at + deployed retention days`. `share_id` SHALL be a foreign key with `ON DELETE CASCADE`, so the owner `public.delete_owner_report` transition removes events atomically with the report/share graph. The store SHALL omit bearer-token plaintext, PIN material, report contents, raw IP addresses, full user-agent strings, and URLs. Malformed or unknown tokens SHALL create no share-scoped row; their aggregate rate-limit telemetry SHALL contain no token, PIN, or share identifier. EH-151 SHALL own hourly retention cleanup; event-write failure SHALL NOT change the authorization result or disclose sensitive input.
...
#### Scenario: Report deletion removes the capability graph

- **WHEN** the durable owner report-delete transition commits for a report with active/replaced shares and access history
- **THEN** the report, share links, explicit document scopes, replacement operations, and share-scoped events are deleted atomically
- **AND** no public capability remains usable
