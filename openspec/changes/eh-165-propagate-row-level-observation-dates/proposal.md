...
Laboratory history tables already persist a per-row `document_extracted_biomarkers.collected_at`, but every acceptance writer stamps `documents.observed_at` instead. Distinct years then share one `observed_at` and EH-129 trends collapse onto a single day. Source-row uniqueness still stores one observation per extracted cell, but the days are wrong. EH-129 trends and EH-126 chronology are only truthful if each extracted value keeps its own collected day.
...
- Add parser/helper fixtures for 1×3 dates, 3×3 dates, header-only dates, undated columns, and same-day repeats that remain two extracted rows.
...
- **Compatibility:** replaying the same extracted row still upserts on `source_extracted_biomarker_id`. Two extracted rows on the same day remain two observations. EH-119 date overrides remain authoritative. Partial `YYYY` / `YYYY-MM` still do not project to 1 January.
