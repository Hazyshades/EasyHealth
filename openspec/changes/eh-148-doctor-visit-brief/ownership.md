...
| EH-151 Scoped Shares | share-link, `share_replacement_operations`, and `report_share_access_events` migrations/repository/retention; `public.replace_report_share` RPC; `src/lib/share-links/**`; owner creation endpoint; public `/share/**` page/API and its export-actions integration slot; `worker/src/index.ts` and `worker/src/env.ts` cleanup integration; public response policy; `QA/eh-151/**` | EH-152 consumes the owner-scoped event read projection and management/replacement repository seam. EH-153 supplies the export-actions component and contract; EH-151 remains the public share-page writer and wires that component. EH-154 inspects the public boundary but does not create a second verifier. |
