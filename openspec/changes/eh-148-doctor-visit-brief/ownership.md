...
|---|---|---|
| EH-148 Doctor Visit Brief | `src/lib/report-contract.ts`; `src/lib/report-evidence.ts`; `src/lib/report-read.ts`; `src/lib/report-safety-policy.ts`; `src/lib/reports.ts`; `src/lib/report-prompts.ts`; `src/lib/generate-doctor-summary.ts`; `src/lib/documents/structured-context.ts`; `src/app/api/reports/route.ts`; `src/app/app/reports/[id]/page.tsx`; `src/components/report-body.tsx`; report contract/validation/evidence persistence migration and `public.create_validated_report` RPC; `QA/eh-148/**` | Contract, source projection, safety policy, read resolver, structured persistence transition/RPC, and report-detail integration are the foundation. The durable-deletion change owns source-known/invalidation columns, tombstone invalidation/final purge, and report-delete grants/transition. EH-150 supplies validator calls to EH-148; EH-153 supplies a leaf export component to the EH-148 page seam. `src/app/api/reports/[id]/route.ts` is owned by the durable-deletion change for the DELETE transition; EH-148 hands off its GET/read contract rather than editing that file in parallel. |
...
| EH-151 Scoped Shares | share-link, `share_replacement_operations`, `share_rate_limit_buckets`, and `report_share_access_events` migrations/repository/retention; service-only replacement, cleanup, and rate-limit RPCs; `src/lib/share-links/**` including `public-response-policy.ts`, `trusted-ingress.ts`, `trusted-ingress-transport.ts`, and `rate-limit.ts`; `openspec/changes/eh-151-scoped-expiring-share-links/deployment/trusted-ingress.yaml`; `src/lib/env.ts`; `.env.example`; owner creation endpoint; public `/share/**` page/API and its export-actions integration slot; `worker/src/index.ts` and `worker/src/env.ts` cleanup integration; public response policy; `QA/eh-151/**` | EH-152 consumes the owner-scoped event read projection and management/replacement repository seam. EH-153 consumes `applyPublicShareResponsePolicy` for public exports and supplies the export-actions component and contract; EH-151 owns the deployment transport handoff and applies the public-page integration patch. EH-154 consumes minimized event/header/deployment evidence. |
...
| Durable document deletion | `src/app/api/documents/[id]/route.ts`; `src/app/api/reports/[id]/route.ts` DELETE transition; document tombstone/final-purge migrations/RPCs; `public.delete_owner_report`; `QA/eh-104/**` | Owns committed document tombstone, report invalidation/final purge, owner report-delete transition, runtime grants, and route cutover. EH-148/EH-151/EH-153 consume the committed lifecycle/read outcomes. |
...
- `src/app/api/reports/[id]/route.ts` has one writer: the durable document-deletion change owns the DELETE transition and EH-148 hands off its read contract; no direct report-table delete remains. EH-148 owns the report detail read surface and generation route.
...
0. Land `make-document-deletion-durable` and its committed tombstone/report-delete/grant handoff before EH-148 report persistence or EH-151/EH-153 public reads/exports.
