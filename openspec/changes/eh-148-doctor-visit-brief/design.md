...
- `validation`, an immutable envelope with `status` (`valid` or `limited` for publishable reports), `version`, and `issue_codes[]` drawn only from the EH-150 code list. Invalid candidates are never persisted; legacy rows have a missing envelope. Issue codes contain no source text, health values, tokens, or profile identifiers.
...
The report-generation request may include an optional `biomarker_dynamics_period` object with inclusive UTC `start` and `end` dates. The server validates the range and exact report document scope, passes the scope plus period to EH-149, and omits `extensions.biomarker_dynamics` when the field is absent. The request never accepts a client DTO or raw observations.
...
`POST /api/reports` resolves eligible documents once, applies the requested selection, and delegates to the service-only `createValidatedReport` transition. The service runs EH-150's pure validator against the server-authorized source catalog and immutable document scope; when `biomarker_dynamics_period` is present, it passes the exact `source_document_ids` scope plus the period to EH-149, whose comparison snapshot excludes candidates from other documents. The transition then calls the EH-148-owned `public.create_validated_report` RPC defined in the report-persistence migration. The RPC accepts only service-generated content, scope, source mappings, an optional scope-checked dynamics extension, and the validation envelope; it rechecks profile ownership, source-row identity, document scope, claim/source relationships, and every dynamics point/document mapping under `SECURITY DEFINER`, then inserts the report, exact UUID array, validation metadata, and `report_evidence_sources` in one database transaction. Any parse, validation, RPC, or persistence failure rolls back.
...
`src/lib/report-read.ts` is the EH-148-owned server read resolver for owner detail, EH-151 public reports, and EH-153 exports. It loads the persisted report, validation envelope, `report_evidence_sources`, and current source rows within the immutable scope; it rejects missing, invalid, unknown-code, or tampered validation envelopes and dynamics extensions whose point/document identity is outside that scope, marks affected claims `limited`, and adds a `SOURCE_UNAVAILABLE` limitation when an observation/finding/note/prescription/referral/document is archived or deleted. It preserves the historical snapshot for display, denies live/raw-source access, and returns the derived read status without rewriting `reports.content`.

### 5. Keep the generator deterministic at the boundary

The prompt asks for structured claim kinds, closed template IDs/parameters, and source IDs, but the server owns normalization and safety: duplicate source references are collapsed, unknown IDs are rejected, the closed template renderer reads only cited source snapshots, and the disclaimer is injected by code. Factual input never supplies renderable text. Removed claims are omitted from the persisted/public claim list; machine issue codes and limitations carry no model prose, and every renderer/exporter filters defensively by `status`. The parser rejects diagnosis, treatment, urgency, imperative, or free-form factual fields; only `clinician_question` may retain model `question_text`, and it is rendered as a question rather than a report fact. The generator cannot add a new source, widen scope, or write a diagnosis/treatment/urgency field.
