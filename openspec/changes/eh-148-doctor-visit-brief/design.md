...
Implementation prerequisite: `make-document-deletion-durable` is a hard prerequisite for this change. That change owns the `documents.lifecycle=deleting` tombstone transaction, `write_generation` increment, `document_deletion_operations` receipt, report invalidation/final purge, and the owner `DELETE /api/documents/:id` cutover. EH-148 consumes the committed tombstone state and its source-set invalidation handoff; it does not implement a second document lifecycle, infer deletion from missing rows, or let owner/share/export reads proceed while the durable deletion operation is pending.
...
- `sections`, an ordered tuple of required containers with the closed IDs `document_summary`, `latest_measurements`, `changes`, `clinician_questions`, `limitations`, and `source_ledger`, in that canonical order; no unknown, duplicate, or missing section ID is valid.
...
- `limitations[]`, each with a machine `id` and limitation code/message safe for display, plus the mandatory educational disclaimer.
...
Every new report contains each canonical section exactly once, and every section object has an `items` array of typed references. Claim-bearing sections use `{ type: "claim_ref", claim_id }`; `limitations` uses `{ type: "limitation_ref", limitation_id }`; and `source_ledger` uses `{ type: "source_ref", source_id }`. Every non-removed claim and every source appears exactly once in a matching reference, every limitation reference resolves, every claim's `section` matches its containing section, and removed claims are absent from persisted/public items. If an array is empty, that section MUST carry an `empty_state`; `document_summary`, `latest_measurements`, `changes`, and `clinician_questions` allow `no_data`, `not_applicable`, or `insufficient_evidence`, while `limitations` and `source_ledger` allow only `no_data`. `limitations` is the machine limitation block and `source_ledger` is derived from persisted sources. Claims may target only compatible sections: `source_fact` is allowed in `document_summary`, `latest_measurements`, or `changes`; `numeric_observation` is allowed in `latest_measurements` or `changes`; `clinician_question` is allowed only in `clinician_questions`; `limitations` and `source_ledger` do not accept model claims. Unavailable required content is represented by a limitation or section empty state, never an omitted container.

The report-generation request may include a bounded `questions` array and an optional inclusive UTC `report_date_range: { start, end }`, in addition to `biomarker_dynamics_period`. Each question is trimmed/NFC-normalized, must contain 1–240 Unicode scalar characters, must contain no control characters or line breaks, and the array contains at most five unique questions. The server stores accepted user questions as `clinician_question` claims with `origin: user_selected`; they are never treated as factual claims or answered by inserting model-authored factual text. `report_date_range.start` and `.end` are canonical `YYYY-MM-DD` UTC calendar dates with `start <= end`; filtering compares the UTC calendar date of each authoritative source date, so the entire end date is inclusive. The server validates the range and applies it to the authoritative source/document date before resolving the immutable scope. The request never accepts client DTOs or raw observations.
...
`POST /api/reports` resolves eligible documents once, applies the requested selection, optional `report_date_range`, and existing `abnormal_only` policy, and captures each resolved source document's current `write_generation` before any LLM work. It delegates to the service-only `createValidatedReport` transition with a typed `requested_scope` (`explicit` plus the requested UUIDs, or `all_eligible`), the exact non-null `source_document_ids` whose data entered the catalog, the per-document generation snapshot, and the generated candidate. With a date range, source eligibility uses the UTC calendar date of the source's authoritative date: observation date for measurements and document date for findings, notes, prescriptions, referrals, and document summaries; undated sources are excluded. A document enters the materialized scope only when it contributes at least one eligible in-range source (or an in-range document-summary source). An explicitly supplied `document_ids` entry that is unauthorized, has no authoritative in-range source, or is outside the date range returns a safe validation error rather than widening or silently changing the requested scope; the all-eligible case includes only documents with an eligible in-range source. The service runs EH-150's pure validator against the server-authorized source catalog and immutable document scope; when `biomarker_dynamics_period` is present, it passes the exact `source_document_ids` scope plus the period to EH-149, whose comparison snapshot excludes candidates from other documents. The transition then calls the EH-148-owned `public.create_validated_report` RPC defined in the report-persistence migration. The RPC locks source documents in sorted UUID order before report keys, rechecks profile ownership, active/not-deleting state, source-row identity, scope, claim relationships, and unchanged `write_generation`, and atomically persists the requested scope, actual source-document IDs, mappings, validated content, optional scope-checked dynamics extension, validation envelope, and generated metadata. Any tombstone/deletion or republish/reprocess generation drift rejects the candidate with no readable report.

The report-persistence migration adds nullable legacy-compatible columns `validation_status text`, `validation_version text`, and `validation_issue_codes text[]`, plus new-report fields for typed `requested_scope_kind`, `requested_document_ids`, and the exact non-null actual `document_ids` source set. New RPC writes require `validation_status` `valid` or `limited`, a non-empty `validation_version`, and issue codes from EH-150's stable list; `invalid` candidates are rejected before insertion. Direct `INSERT`/`UPDATE`/`DELETE` on `public.reports` and direct writes to `report_evidence_sources` are revoked from `service_role`, `authenticated`, `anon`, and `PUBLIC`; the named EH-148 `public.create_validated_report` SECURITY DEFINER transition is the only new-report/evidence writer, while the durable-deletion change owns `public.delete_owner_report` for owner deletion. `report-read.ts` maps these columns into the immutable public `validation` envelope, and EH-151/EH-153 consume that projection rather than choosing their own fields.

The migration revokes `EXECUTE` on `public.create_validated_report` from `anon`, `authenticated`, and `public` and grants it only to `service_role`; the service role receives no direct table-DML grant. The durable-deletion change owns the separate owner-authorized `public.delete_owner_report` transition and the application DELETE route. Neither RPC trusts a client-supplied profile or scope without re-resolving ownership and source identity inside the function.

The persisted JSON contains the source snapshots used to render the report. The `report_evidence_sources` rows are written in the same transaction as the report, cascade with report deletion, and are not exposed in public DTOs. A later archive/removal of a source row while its parent document remains active can disable a live source link without changing the historical text already shown; a document tombstone instead invalidates the complete report and schedules whole-report purge before any snapshot is read. No source snapshot is used to authorize a new document download.
...
`src/lib/report-read.ts` is the EH-148-owned server read resolver for owner detail, EH-151 public reports, and EH-153 exports. It loads the persisted report, validation envelope, `report_evidence_sources`, and current source rows within the immutable scope. For a legacy row identified by a missing contract version or null scope, it returns a `legacy` owner presentation that remains readable but explicitly disables sharing/export; EH-151/EH-153 reject that result generically. For a new structured row, it rejects a missing, invalid, unknown-code, or tampered validation envelope and dynamics extensions whose point/document identity is outside that scope. If a source row is archived or removed while its parent document remains active, it marks affected claims `limited`, adds a `SOURCE_UNAVAILABLE` limitation, preserves the historical snapshot for display, denies live/raw-source access, and returns the derived read status without rewriting `reports.content`. If a source document enters `deleting`/tombstoned state, the report is invalidated as a whole before content is read, owner/share/export resolvers return generic unavailable, and the durable deletion path marks it for final purge; no snapshot or report bytes are exposed after the tombstone.
...
The file-level ownership matrix is recorded in `openspec/changes/eh-148-doctor-visit-brief/ownership.md`. EH-148 owns `report-contract.ts`, source projection, report persistence and detail integration. EH-150 owns the validator module and returns a stable validation result; EH-148 owns the generation-route integration point. EH-149 owns biomarker dynamics projection, EH-151 owns public share verification and trusted-ingress integration, EH-152 owns share management UI, and EH-153 owns export adapters. `make-document-deletion-durable` owns document tombstone/final purge, report invalidation, the owner report-delete transition and runtime grant cutover; EH-148 owns the consuming report-read contract. No parallel branch edits another branch's owner files; cross-cutting changes are handed off through the contract.
