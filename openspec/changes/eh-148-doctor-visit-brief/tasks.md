...
- [ ] 1.1 Add the versioned `DoctorVisitBrief` and `ReportEvidenceRef` schemas in `src/lib/report-contract.ts`, including claim status, question origin (`generated`/`user_selected`), bounded user-selected questions, source kinds, limitations, and the educational disclaimer.
...
- [ ] 2.2 Integrate structural parsing, EH-150 validation, the server-resolved inclusive report date range, and the optional `biomarker_dynamics_period` plus exact report-scope handoff to EH-149 into the service-only `createValidatedReport` transition; accept only the server-generated frozen extension and validation envelope, then call the EH-148-owned `public.create_validated_report` RPC with service-generated content, normalized question claims, filtered scope, source mappings, validator status/version/issue codes, and generated-at metadata.
...
- [ ] 3.1 Replace free-form report rendering with typed Doctor Visit Brief sections, citations, limitations, a source ledger, and visibly distinct generated versus user-selected clinician questions.
...
- [ ] 4.1 Add focused verification for mixed source kinds, inclusive report date range and explicit-scope conflicts, bounded/invalid user questions, exact scope, unknown citation/template IDs, unsafe model-authored factual text, validation-envelope status/version/issue-code cases, report-scope-constrained dynamics, missing evidence, archive/delete-after-publication, legacy rows, question rendering, and educational wording.
