...
1. **Schema:** version, the EH-148 closed section IDs/order (`document_summary`, `latest_measurements`, `changes`, `clinician_questions`, `limitations`, `source_ledger`), exactly one container for each required section with an `items` array of typed references (`claim_ref` for the first four, `limitation_ref` for limitations, `source_ref` for the source ledger) and an `empty_state` whenever empty, allowed empty states (`no_data`, `not_applicable`, `insufficient_evidence` for the first four sections; `no_data` only for limitations/source ledger), claim IDs, EH-148's closed claim-input contract, approved template IDs and exact template parameters, source IDs, citation shape, allowed source kinds, and rejection of diagnosis/treatment/urgency/imperative/free-form factual fields. Factual `text` is invalid input; only EH-148's server renderer may create it. Every non-removed claim/source/limitation is referenced exactly once, every claim's `section` matches its containing section, and claim-kind/section compatibility is enforced: `source_fact` only in `document_summary`/`latest_measurements`/`changes`, `numeric_observation` only in `latest_measurements`/`changes`, and `clinician_question` only in `clinician_questions`; limitations and source ledger are not model-claim sections.
