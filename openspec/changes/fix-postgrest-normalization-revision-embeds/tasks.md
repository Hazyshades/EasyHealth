...
- [x] 1.1 Add the temporary old-name composite FK alias with the same columns, target key, MATCH FULL, delete behavior, and deferrability as `observations_normalization_revision_same_source_fk`.
- [x] 1.2 Add preflight that identifies whether migration 034 is applied, whether each constraint exists, and whether any relationship definition diverges.
- [x] 1.3 Add schema-cache reload and live old-hint recovery checks for the 034-applied path.
- [x] 1.4 Add the controlled pause/apply-034-plus-bridge/cache-reload procedure for the 034-pending path.

## 2. Runtime consumers

- [x] 2.1 Update document observations to the same-source FK hint.
- [x] 2.2 Update Biomarkers and Health Profile to the same-source FK hint.
- [x] 2.3 Update Reports and structured context to the same-source FK hint.
- [x] 2.4 Add a scoped static check that bans the old hint in active runtime code while allowing migrations/history.
