...
- [ ] 10.5 Final apply readiness: all slice gates green, audit findings closed or explicitly deferred with owner

Blocked on 10.5: the candidate input hash moved with the corpus and policy
change, so the seven hash-bound approvals in
`registry/candidate-release/v1/approvals.json` must be re-issued by their named
owners before `check:registry-v2-candidate-corpus` (launchable) passes. The
technical gate `check:registry-v2-candidate-corpus-technical` passes.
