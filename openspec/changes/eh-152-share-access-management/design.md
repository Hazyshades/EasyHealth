...
The management list never returns a link or token. The EH-151 creation response is held only in component state, can be copied once, and is cleared when the page is left. For an existing active share, the UI offers **Create replacement link**, not copy-from-list. `POST /api/share-management/[id]/rotate` authenticates the owner, mints a new random token and keyed digest in the server process, and calls the EH-151 service-only `public.replace_report_share` RPC with the predecessor ID, owner/share-scoped idempotency key, new digest, and key version. The `SECURITY DEFINER` RPC locks the predecessor, reserves the unique operation row, copies the immutable report scope, raw-document allow-list, expiry, and export policy into exactly one successor, revokes the predecessor, and commits all writes together. If minting or validation fails, no RPC call is made or the transaction rolls back and the old link remains active. A replay of the same idempotency key returns the stored operation result without minting another token; a different concurrent request sees the revoked predecessor and returns a conflict without creating a second successor. Only the first successful response contains the new plaintext link. Analytics receives a boolean outcome and share ID, never the URL or token.
