...
const deletionMigration = source(
...
assert.match(
  ...
  "document DELETE must enqueue a durable tombstone operation",
);
assert.doesNotMatch(
  documentRoute,
  /purgeDocumentDerivedLaboratoryLineage/,
  "document DELETE must not synchronously purge laboratory lineage",
);
assert.match(
  deletionMigration,
  /finalize_document_deletion[\s\S]*delete from public\.documents/,
  "durable finalizer must purge the document after derived lineage",
);
assert.match(
  deletionMigration,
  /easyhealth\.purge_lineage/,
  "durable finalizer must use the established controlled purge context",
);
