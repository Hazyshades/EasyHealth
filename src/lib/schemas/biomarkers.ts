...
export function parseRawExtraction(raw: unknown): ExtractionResult {
  ...
  for (const item of list) {
    ...
    const name =
      typeof row.name === "string" && row.name.trim()
        ...
        : "Unknown test";
    const keySource =
      typeof row.key === "string" && row.key.trim() ? row.key : name;
  ...
  return {
    lab_name:
      typeof data.lab_name === "string" && data.lab_name.trim()
        ? data.lab_name.trim()
        : null,
    observed_at:
      typeof data.observed_at === "string" && data.observed_at.trim()
        ? data.observed_at.trim()
        : null,
    biomarkers,
  };
}
...
function stripModelReasoningPreamble(text: string): string {
  ...
  cleaned = cleaned.replace(
    new RegExp(`${thinkOpen}[\\s\\S]*?${thinkClose}`, "gi"),
    "",
  );
...
export function parseJsonFromModelText(text: string): unknown {
  ...
  function tryParseBalanced(
    ...
    for (
      let start = trimmed.indexOf(openChar);
      start >= 0;
      start = trimmed.indexOf(openChar, start + 1)
    ) {
      let depth = 0;
      let inString = false;
      let escape = false;
