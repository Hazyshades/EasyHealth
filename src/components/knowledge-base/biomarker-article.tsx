  ...
  const aliases = Array.from(
    new Set(
      article.measurementDefinitions.flatMap((definition) =>
        definition.aliases
          .filter(
            (alias) => alias.locale === "en" && alias.lifecycle === "active",
          )
  ...
  const specimens = Array.from(
    new Set(
      article.measurementDefinitions.map((definition) =>
        formatKnowledgeSpecimen(definition.specimen),
      ),
    ),
  );
  ...
  const relatedMeasurements = article.relatedMeasurementKeys.map((key) => ({
    ...
    article: publishedArticles.find(
      (candidate) =>
        ...
        candidate.measurementDefinitionKeys.includes(key),
    ),
  }));
    ...
    <div className="min-h-screen bg-[var(--eh-canvas)] text-[var(--eh-text-primary)]">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-8 sm:px-6 sm:pt-12 lg:px-8">
        <nav
          ...
          className="mb-6 text-sm text-[var(--eh-text-secondary)]"
        >
          <Link
            href="/knowledge"
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
          <div className="space-y-6">
            ...
            <section
              ...
              aria-labelledby="registry-metadata-heading"
            >
              <div className="mb-5 flex items-baseline justify-between gap-4">
                <h2
                  id="registry-metadata-heading"
                ...
                <div>
                  ...
                  <dd className="mt-2 space-y-1">
                    {article.measurementDefinitions.map((definition) => (
                      <code
                        key={definition.key}
                        className="block break-all font-mono text-xs text-[var(--eh-text-primary)]"
                      >
                        {definition.key}
                      </code>
                    ))}
            ...
            <section
              className="rounded-2xl border border-sky-200 bg-sky-50 p-5 sm:p-7"
            ...
            >
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-sky-900">
                ...
                diagnosis, or treatment. Laboratory results need the reporting
                laboratory&apos;s context and a qualified healthcare
                professional&apos;s interpretation. Do not use this page to make
          ...
          <aside className="space-y-6 lg:sticky lg:top-6">
            <section
              className="rounded-2xl border border-[var(--eh-brand)]/20 bg-[var(--eh-brand-soft)] p-5"
              aria-labelledby="private-data-heading"
            >
              <p className="text-xs font-medium uppercase tracking-[0.1em] text-[var(--eh-brand)]">
                Private workspace
              </p>
              <h2
                id="private-data-heading"
                className="mt-2 text-lg font-semibold text-[var(--eh-text-primary)]"
              >
                Your EasyHealth data
              </h2>
              <p className="mt-2 text-sm leading-6 text-[var(--eh-text-secondary)]">
                Open your own accepted observations separately. This public
                guide never includes personal results.
              </p>
              <div className="mt-4 space-y-2">
                {article.measurementDefinitions.map((definition) => (
                  <Link
                    key={definition.key}
                    href={buildHealthNavigationPath("/app/biomarkers", {
            ...
            {relatedPanels.length > 0 ? (
              <section
                className="rounded-2xl border border-[var(--eh-border)] bg-white p-5"
                aria-labelledby="related-panels-heading"
              >
                <h2
                  id="related-panels-heading"
                  className="text-lg font-semibold text-[var(--eh-text-primary)]"
                >
                  Related reading
                </h2>
                <ul className="mt-4 space-y-3">
                  {relatedPanels.map((panel) => (
                    <li key={panel.key} className="text-sm leading-5">
                      <Link
            ...
            >
              ...
              <ul className="mt-4 space-y-3">
                {relatedMeasurements.map(
                  ({ key, definition, article: relatedArticle }) => (
            ...
            <section
              className="rounded-2xl border border-[var(--eh-border)] bg-white p-5 text-sm text-[var(--eh-text-secondary)]"
            ...
            >
              ...
              <dl className="mt-4 space-y-3">
                <div>
                  <dt className="text-xs uppercase tracking-[0.08em] text-[var(--eh-text-muted)]">
                    Reviewed by
                  </dt>
