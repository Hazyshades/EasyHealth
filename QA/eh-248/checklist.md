...
- [ ] `pnpm test:eh122-db` — migration constraints, RPC signatures, trusted writer validation, restore CAS, reprocess fact propagation, and history triggers. **Blocked locally:** Docker Desktop's Linux engine is unavailable. The latest CI runs exposed the replay-order regression, a stale `profiles.wallet_address` fixture, a malformed pgTAP `has_column` wrapper, and an incomplete writer overload argument list; all are corrected locally and require another CI run after commit.
