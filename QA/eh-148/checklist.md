...
- [ ] Tombstone/race evidence proves a deleting source document invalidates the complete report before owner/share/export bytes are read, and document-first writer locking plus `write_generation` revalidation rejects tombstone/republish races with no stale report committed or served. *(Evidence provider: EH-148 RPC/read-resolver owner; durable-deletion owner; EH-151 share owner; EH-153 export owner.)*
