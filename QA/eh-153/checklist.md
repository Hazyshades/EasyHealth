...
- [ ] PDF renderer/font verification covers Unicode, long content, canonical EH-148 section order, allowed empty sections, and explicit failure. *(Evidence provider: EH-153 serializer owner.)*
...
- [ ] Source-unavailable evidence proves the export consumes the committed `make-document-deletion-durable`/EH-148 tombstone outcome: archive/remove-after-publication exports preserve the historical snapshot and `SOURCE_UNAVAILABLE` limitation only while the parent document remains active, while tombstoned source documents invalidate the complete report before serialization and deny all bytes. *(Evidence provider: durable-deletion owner; EH-148 read-resolver owner; EH-153 export owner; EH-151 public-read owner.)*
...
- [ ] Validation-envelope evidence proves valid/limited reports serialize only safe status/version projections and visible limitations, while internal issue codes never enter owner or shared export bytes; invalid, legacy, missing, or tampered envelopes fail closed before bytes are returned. *(Evidence provider: EH-148 read-resolver owner; EH-150 validator owner; EH-153 adapter/serializer owner; EH-151 share owner.)*
- [ ] Section-contract evidence proves unknown, duplicate, or missing section containers fail before bytes, allowed empty states remain explicit, and PDF/CSV/JSON use the canonical order `document_summary`, `latest_measurements`, `changes`, `clinician_questions`, `limitations`, `source_ledger`. *(Evidence provider: EH-148 contract owner; EH-150 validator owner; EH-153 serializer owner.)*
