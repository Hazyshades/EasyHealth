...
- [x] `pnpm test:registry-v2-candidate-corpus` — passed on 2026-08-03: `verify-registry-v2-candidate-corpus: all checks passed`. It proves the committed candidate has every release artifact and satisfies every technical threshold; it does not treat human governance as a technical failure.
- [ ] `pnpm check:registry-v2-candidate-corpus` — blocked on 2026-08-03 only by hash-bound release approvals. The typed-catalog report has 100% raw preservation, recognition, expected-classification, alias, and unit coverage; zero false concrete resolutions; zero processing errors; and no uncovered rows. All technical threshold checks pass for candidate input `c409867956e5e30e9c96a6aa693b1406ae325829cfd584c2692b3ebc49b8c481`. The candidate remains non-launchable until fresh approvals for that exact hash are supplied by the Registry Safety Reviewer, Assessment Owners for `alt_serum_catalytic_activity` and `glucose_serum`, and Release Manager. Do not alter approval hashes without those reviews.
- [x] Local disposable database verification — passed on 2026-08-03. `npx supabase db reset --local` applied migrations `001`–`040`, including `040_eh113_cbc_method_evidence.sql`. `npx supabase test db --local supabase/tests/eh113_cbc_method_evidence.sql` passed: `Files=1, Tests=5, Result: PASS`. It proves that `document_extracted_biomarkers.method` exists, remains nullable, accepts `manual` and `automated`, and rejects unsupported method evidence.
- [x] The local disposable run proves migration `040_eh113_cbc_method_evidence.sql` applies cleanly and enforces its database contract. The remote sandbox remains out of sync at migration `024`; it is not used as EH-113 verification evidence.
