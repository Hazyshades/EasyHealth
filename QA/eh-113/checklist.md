...
- [x] `npm run test:measurement-registry` — passed on 2026-07-30: `verify-measurement-registry: all checks passed`. Covers registry validation and the incomplete-resolution projection contract.
- [x] `npm run test:eh113` — passed on 2026-07-30: `verify-eh113-cbc-launch-catalog: all checks passed`. Covers the reviewed CBC fixture matrix: five-part percent/absolute forms, RDW variants, reticulocyte forms, platelet indices, manual segmented/band populations, multilingual aliases, missing axes, unit conflicts, and an unsupported OCR negative.
- [x] `npm run test:registry-v2-runtime` — passed on 2026-07-30: `verify-registry-v2-runtime-cutover: all checks passed`. Covers active, synchronized Registry 2.0 runtime bindings.
- [x] `npm run typecheck` — passed on 2026-07-30 with no diagnostics. Covers compilation of the extraction, resolver, reader query, and fixture changes.
...
- [ ] `npm run test:registry-v2-candidate-corpus` — blocked on 2026-07-30. EH-113 CBC classification and alias coverage are 100%, with zero unmapped CBC rows and zero false concrete resolutions. The shared release gate remains non-launchable: its global unit-coverage metric is 72.7% because 12 existing non-CBC corpus rows lack compatible catalog definitions, and it also requires new hash-bound approvals after the candidate input hash changed to `0fc9692cfb4b3aea8abb4485c5b00d0e3fd71e94bb9db103fd3e3c5547c53765`. Required approvers: Registry Safety Reviewer, Assessment Owner for `alt_serum_catalytic_activity` and `glucose_serum`, and Release Manager. Do not alter approval hashes without their review.
