...
- [x] Clean disposable Supabase database contract passed: `supabase test db --local supabase/tests/eh126_normalized_medical_events.sql` (26 pgTAP assertions) covering event creation, backfill-compatible ownership, date roles and validation, sync RPC, nullable projections, observation linkage, profile scoping, and deterministic database ordering.
...
- [x] Clean-database `supabase test db --local supabase/tests/eh121_observation_change_history.sql` passed (37 assertions) after the EH-126 document/event unlink cascade guard was added.
