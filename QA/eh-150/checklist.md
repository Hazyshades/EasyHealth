...
- [ ] Section-contract fixtures prove the EH-148 canonical six-section order, exactly one required container each, unknown/duplicate/missing/incompatible failures with `SCHEMA_INVALID`, and explicit allowed empty states with visible limitations. *(Evidence provider: EH-148 contract owner; EH-150 validator owner; EH-153 serializer owner.)*
- [ ] Read-time lifecycle status is supplied by EH-148's resolver after the committed `make-document-deletion-durable` tombstone state: archived/removed source rows under active documents become `SOURCE_UNAVAILABLE` limited snapshots, while a tombstoned source document invalidates the complete report before owner/share/export bytes; EH-150 does not authorize live/raw-source access from a historical snapshot. *(Evidence provider: durable-deletion owner; EH-148 read-resolver owner; EH-150 validator owner.)*
