...
- **UI smoke:** `Passed`. The product owner manually exercised the interface checks above with the synthetic/de-identified fixtures: exact same-page preview, keyboard focus parity, pinned-plus-preview coexistence, cross-page page-local fallback, fuzzy/ambiguous/legacy/scanned fallback, multiline precision across zoom/resize, accepted-observation fallback, and unavailable-preview retry behavior. The earlier unauthenticated local-shell limitation is superseded by this manual result.
