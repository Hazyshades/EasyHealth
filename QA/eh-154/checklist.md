...
- [ ] Deployed rate-limit evidence proves EH-151's Postgres-backed `rate-limit.ts` adapter uses the recorded HMAC pepper and `10/60s` token / `30/60s` requester settings, returns generic `429` on exhaustion and `503` on store failure, has no local fallback, persists no raw token/IP/user-agent values, and drains expired buckets through the bounded worker cleanup RPC. *(Evidence provider: EH-151 rate-limit owner; EH-154 harness/gate owner.)*
