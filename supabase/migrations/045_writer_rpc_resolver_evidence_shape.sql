-- #117: accept the decision-trace object the writer actually sends.
--
-- Acceptance had never succeeded. The writer's delegate required
-- `resolver_evidence` to be a JSON array, a rule inherited from migration 021
-- when the column held a flat evidence list. Since EH-106 the writer sends the
-- v2 `ResolverDecisionTrace` object, so every row failed with
-- `invalid_normalization_resolution_payload`, the API reported only a
-- placeholder, and the database held zero observations.
--
-- Only the validation was stale. The object is the correct shape:
-- `observation-read-boundaries` reads `.selectedCandidateKey` and `.outcome`
-- from this column and `incomplete-laboratory-outcomes` reads `.candidates`.
--
-- Note the target: EH-115 (migration 039) renamed the EH-106 function to
-- `_legacy` and placed a trace-validating wrapper in front of it under the
-- original name. The stale check lives in the delegate, so that is what is
-- recreated here — replacing the wrapper would revert EH-115.
--
-- Covered by supabase/tests/writer_rpc_seam.sql, which submits the exact payload
-- `buildNormalizationResolutionPayload` produces.
